...
def analyze_query(user_input: str, memory: list) -> Dict[str, Optional[str]]:
    ...
    analysis_prompt = (
        ...
        "Decide if this is a social interaction (includes greetings, farewells, pleasantries, small talk, etc.), a complete support question, a query that is missing information, or a follow-up question. \n"
        "Extract model name and model number if present.\n"
        "Return JSON with:\n"
        "- intent: social_interaction | support_question | unknown | follow_up_question\n"
        "- model_name (if any)\n"
        "- model_number (if any)\n"
